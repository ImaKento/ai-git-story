...
const CONFIG_DIR = path.join(os.homedir(), ".ai-git-story");
...
export function saveConfig(config: Partial<Config>): void {
  const current = loadConfig() || { language: "ja" };
...
/**
 * モデル名を取得（設定ファイル → 環境変数 → デフォルト）
 */
export function getModel(): string {
  const config = loadConfig();
  return config?.model || process.env.GROQ_MODEL || "llama-3.3-70b-versatile";
}
